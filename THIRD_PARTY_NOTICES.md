# Avisos de terceros

Inventario generado desde el entorno de build de CuadernoPro v8.4.8.
Incluye dependencias directas, transitivas y herramientas cuyos
componentes se incorporan al ejecutable Windows. `pip` y `setuptools`
se excluyen porque solo preparan el entorno y no se distribuyen.

Las licencias pertenecen a sus respectivos titulares. Este inventario
no sustituye los textos de licencia originales incluidos por cada
proyecto ni cambia la licencia GPL-3.0-or-later de CuadernoPro.

## Paquetes Python y de empaquetado

| Paquete | Versión | Licencia | Fuente |
| --- | ---: | --- | --- |
| altair | 6.2.2 | BSD-3-Clause | https://github.com/vega/altair |
| altgraph | 0.17.5 | MIT | https://github.com/ronaldoussoren/altgraph |
| anyio | 4.14.1 | MIT | https://anyio.readthedocs.io/en/latest/ |
| attrs | 26.1.0 | MIT | https://www.attrs.org/ |
| blinker | 1.9.0 | MIT | https://github.com/pallets-eco/blinker |
| branca | 0.8.2 | MIT | https://github.com/python-visualization/branca |
| cachetools | 7.1.4 | MIT | https://github.com/tkem/cachetools/ |
| certifi | 2026.6.17 | MPL-2.0 | https://github.com/certifi/python-certifi |
| charset-normalizer | 3.4.9 | MIT | https://charset-normalizer.readthedocs.io/ |
| click | 8.4.2 | BSD-3-Clause | https://github.com/pallets/click/ |
| colorama | 0.4.6 | BSD-3-Clause | https://github.com/tartley/colorama |
| et_xmlfile | 2.0.0 | MIT | https://foss.heptapod.net/openpyxl/et_xmlfile |
| folium | 0.20.0 | MIT | https://github.com/python-visualization/folium |
| geopandas | 1.1.4 | BSD-3-Clause | https://github.com/geopandas/geopandas |
| gitdb | 4.0.12 | BSD-3-Clause | https://github.com/gitpython-developers/gitdb |
| GitPython | 3.1.51 | BSD-3-Clause | https://github.com/gitpython-developers/GitPython |
| h11 | 0.16.0 | MIT | https://github.com/python-hyper/h11 |
| httptools | 0.8.0 | MIT | https://github.com/MagicStack/httptools |
| idna | 3.18 | BSD-3-Clause | https://github.com/kjd/idna |
| itsdangerous | 2.2.0 | BSD-3-Clause | https://github.com/pallets/itsdangerous |
| Jinja2 | 3.1.6 | BSD-3-Clause | https://github.com/pallets/jinja/ |
| jsonschema | 4.26.0 | MIT | https://github.com/python-jsonschema/jsonschema |
| jsonschema-specifications | 2025.9.1 | MIT | https://github.com/python-jsonschema/jsonschema-specifications |
| lxml | 6.1.1 | BSD-3-Clause | https://github.com/lxml/lxml |
| MarkupSafe | 3.0.3 | BSD-3-Clause | https://github.com/pallets/markupsafe/ |
| narwhals | 2.23.0 | MIT | https://github.com/narwhals-dev/narwhals |
| numpy | 2.5.1 | BSD-3-Clause AND 0BSD AND MIT AND Zlib AND CC0-1.0 | https://github.com/numpy/numpy |
| openpyxl | 3.1.5 | MIT | https://foss.heptapod.net/openpyxl/openpyxl |
| owslib | 0.36.0 | BSD-3-Clause | https://github.com/geopython/OWSLib |
| packaging | 26.2 | Apache-2.0 OR BSD-2-Clause | https://github.com/pypa/packaging |
| pandas | 3.0.3 | BSD-3-Clause | https://pandas.pydata.org |
| pefile | 2024.8.26 | MIT | https://github.com/erocarrera/pefile |
| pillow | 12.3.0 | MIT-CMU | https://github.com/python-pillow/Pillow |
| protobuf | 7.35.1 | BSD-3-Clause | https://developers.google.com/protocol-buffers/ |
| pyarrow | 25.0.0 | Apache-2.0 | https://arrow.apache.org/ |
| pydeck | 0.9.3 | Apache-2.0 | https://github.com/visgl/deck.gl/tree/master/bindings/pydeck |
| pyinstaller | 6.21.0 | GPL-2.0-or-later con excepción de distribución | https://pyinstaller.org |
| pyinstaller-hooks-contrib | 2026.6 | GPL-2.0-or-later; runtime hooks Apache-2.0 | https://github.com/pyinstaller/pyinstaller-hooks-contrib |
| pyogrio | 0.13.0 | MIT | https://github.com/geopandas/pyogrio |
| pypdf | 6.14.2 | BSD-3-Clause | https://github.com/py-pdf/pypdf |
| pyproj | 3.7.2 | MIT | https://pyproj4.github.io/pyproj/ |
| python-dateutil | 2.9.0.post0 | Apache-2.0 OR BSD-3-Clause | https://github.com/dateutil/dateutil |
| python-docx | 1.2.0 | MIT | https://github.com/python-openxml/python-docx |
| python-multipart | 0.0.32 | Apache-2.0 | https://github.com/Kludex/python-multipart |
| pywin32-ctypes | 0.2.3 | BSD-3-Clause | https://github.com/enthought/pywin32-ctypes |
| PyYAML | 6.0.3 | MIT | https://pyyaml.org/wiki/PyYAMLDocumentation |
| referencing | 0.37.0 | MIT | https://github.com/python-jsonschema/referencing |
| reportlab | 5.0.0 | BSD-3-Clause | https://www.reportlab.com/ |
| requests | 2.34.2 | Apache-2.0 | https://github.com/psf/requests |
| rpds-py | 2026.6.3 | MIT | https://github.com/crate-py/rpds |
| shapely | 2.1.2 | BSD-3-Clause | https://github.com/shapely/shapely |
| six | 1.17.0 | MIT | https://github.com/benjaminp/six |
| smmap | 5.0.3 | BSD-3-Clause | https://github.com/gitpython-developers/smmap |
| starlette | 1.3.1 | BSD-3-Clause | https://github.com/Kludex/starlette |
| streamlit | 1.59.1 | Apache-2.0 | https://streamlit.io |
| streamlit-folium | 0.27.2 | MIT | https://github.com/randyzwitch/streamlit-folium |
| tenacity | 9.1.4 | Apache 2.0 | https://github.com/jd/tenacity |
| toml | 0.10.2 | MIT | https://github.com/uiri/toml |
| typing_extensions | 4.16.0 | PSF-2.0 | https://github.com/python/typing_extensions |
| tzdata | 2026.3 | Apache-2.0 | https://github.com/python/tzdata |
| urllib3 | 2.7.0 | MIT | https://urllib3.readthedocs.io |
| uvicorn | 0.51.0 | BSD-3-Clause | https://github.com/Kludex/uvicorn |
| watchdog | 6.0.0 | Apache-2.0 | https://github.com/gorakhargosh/watchdog/ |
| websockets | 16.1 | BSD-3-Clause | https://github.com/python-websockets/websockets |
| xlrd | 2.0.2 | BSD | http://www.python-excel.org/ |
| xyzservices | 2026.3.0 | BSD-3-Clause | https://github.com/geopandas/xyzservices |

## Servicios y datos externos no incluidos

RainViewer no es una biblioteca incorporada al ejecutable. Sus
teselas se consultan opcionalmente en tiempo de ejecución y están
sujetas a condiciones propias. La atribución visible es obligatoria
y la API pública se describe para uso personal, educativo y
comunitario a pequeña escala, sin SLA. Condiciones verificadas el
12/07/2026: https://www.rainviewer.com/api.html

Las atribuciones de SIGPAC, PNOA/IGN, OpenStreetMap y RainViewer se
detallan en `ATRIBUCIONES_DATOS.md`.
